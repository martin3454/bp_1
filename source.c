...
void font_vypis(){
	...
	for(uint8_t i = 0; i < 64; i += 8){
		...
		for(uint8_t k = pom; k < pom + 120; k += 5){
...
void WriteChar(char charToWrite){
	...
	for(i = 0; i < 5; i++){
		if(barva) Glcd_WriteData(ReadByteFromRom((char *)((int)font + (5 * charToWrite) + i)));
		else Glcd_WriteData(~ReadByteFromRom((char *)((int)font + (5 * charToWrite) + i)));
	}
	if (barva)Glcd_WriteData(0x00);
	else Glcd_WriteData(0xff);
}
